...
col_fun = colorRamp2(c(0, 1, 2,3,4, 5, 7), c(helper[1], helper[3],helper[5], helper[6], helper[7], helper[8], helper[9])) #TODO: change depending on the highest number of occurence of a TF

#color activity 
helper = rev(brewer.pal(9, "Greys"))
col_fun2 = colorRamp2(c(0.0, 0.2, 0.5, 0.75,  1, 2, 3), c(helper[1], helper[3], helper[5], helper[6], helper[7], helper[8], helper[9])) #TODO: change depending on the activity

#color cellTypeScore
helper = rev(brewer.pal(11, "RdYlBu"))
col_fun3 = colorRamp2(c(-0.03, -0.02, -0.015, -0.01, 0, 0.01 ,0.015 ,0.02, 0.025, 0.03), c(helper[1], helper[3],helper[4], helper[5], helper[6], helper[7], helper[8], helper[9], helper[10], helper[11])) #TODO: change dependening on the min and max value for the cellTypeScore

pdf(paste(outputDir, "heatmap.pdf", sep = ""))
Heatmap(data_heatmap, name = "?",
	heatmap_legend_param = list(title = "", at = c(0, 1, 2, 3, 4, 5, 7)), #TODO: also change the legend here for the number of TFs
	cluster_rows = as.dendrogram(cluster_REMs), 
	cluster_columns = as.dendrogram(cluster_TFs),
	col = col_fun, 
	column_title = "Transcription factors", 
	column_title_gp = gpar(fontsize = 10),
	column_names_gp = gpar(fontsize = 3), #TODO: maybe change fontsize, dependent on thenumber of TFs shown in the heatmap
	column_title_side = "bottom", #"top"
	row_title = "# TF hits per REM", 
	show_row_names = TRUE,
	row_title_gp = gpar(fontsize = 10),
	row_names_gp = gpar(fontsize = 4), #TODO: maybe change fontsize, dependent on the number of REMs shown in the heatmap
